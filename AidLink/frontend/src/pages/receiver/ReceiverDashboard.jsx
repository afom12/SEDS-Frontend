...
import { FaFileAlt, FaClipboardCheck, FaHandHoldingHeart, FaArrowRight, FaPlus, FaSyncAlt, FaSpinner, FaComments, FaBell } from 'react-icons/fa';
...
const ReceiverDashboard = () => {
  ...
  const loadReceiverData = async ({ silent = false } = {}) => {
    try {
      if (silent) {
        setRefreshing(true);
      } else {
    ...
    <div className="p-6 md:p-8">
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            onClick={() => loadReceiverData({ silent: true })}
            disabled={refreshing}
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        ...
        <Card className="hover:shadow-xl transition-shadow cursor-pointer">
          <Link to="/receiver/messages" className="block">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-xl font-semibold text-text-dark mb-2">
                  {t('receiver.dashboard.messages', 'Messages')}
                </h3>
                <p className="text-gray-600 text-sm mb-4">
                  {t('receiver.dashboard.messagesDesc', 'Secure communication with donors and coordinators')}
                </p>
                <span className="text-primary font-medium flex items-center">
        ...
        <Card className="hover:shadow-xl transition-shadow cursor-pointer">
          <Link to="/receiver/notifications" className="block">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-xl font-semibold text-text-dark mb-2">
                  {t('receiver.dashboard.notifications', 'Notifications')}
                </h3>
                <p className="text-gray-600 text-sm mb-4">
                  {t('receiver.dashboard.notificationsDesc', 'Updates on request review and delivery')}
                </p>
                <span className="text-primary font-medium flex items-center">
                  {t('common.viewAll')} <FaArrowRight className="ml-2" />
                </span>
